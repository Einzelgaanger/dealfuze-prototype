...
import { SelectBoxesComponent } from "@deal-fuze/server";
import { BaseEditor, DefaultElements } from "./base-editor";
...
export function SelectBoxesEditor({
  component,
  isMatching,
  onSave,
  onChangeType,
  compatibleComponents = [],
}: OptionsComponentEditorProps<SelectBoxesComponent>) {
  ...
  const addOption = () => {
    setCurrentComponent({
      ...
      component: {
        ...
        values: [
          ...(currentComponent.component.values ?? []),
  ...
  const removeOption = (index: number) => {
    const values = [...(currentComponent.component.values ?? [])];
  ...
  const validateMinSelectedCount = (minCount: number | undefined) => {
    if (currentComponent.component.values && minCount) {
      const valuesLength = currentComponent.component.values?.length || 0;
      return Math.min(minCount, valuesLength);
    }
    return undefined;
  };
      ...
      nav={
        <div className="flex gap-4">
          <div className="space-y-2 pr-4">
            ...
            <Button
              variant={currentTab === "validation" ? "secondary" : "ghost"}
              className="w-full justify-start text-xs"
      ...
      <div className="flex-1">
        ...
        {currentTab === "data" && (
          <div>
            ...
            >
              {currentComponent.component.values?.map((option, index) => (
        ...
        {currentTab === "validation" && (
          <div className="flex gap-8">
            <div>
              <Label htmlFor="minSelectedCount">Min Selected Count</Label>
              <Input
                type="number"
                value={currentComponent.component.validate?.minSelectedCount}
                onChange={(e) => {
                  setCurrentComponent({
                    ...currentComponent,
                    component: {
                      ...
                      validate: {
                        ...
                        minSelectedCount: validateMinSelectedCount(
            ...
            <div>
              <Label htmlFor="maxSelectedCount">Max Selected Count</Label>
              <Input
                id="maxSelectedCount"
                type="number"
                value={currentComponent.component.validate?.maxSelectedCount}
                onChange={(e) =>
                  setCurrentComponent({
                    ...currentComponent,
                    component: {
                      ...
                      validate: {
                        ...currentComponent.component.validate,
                        maxSelectedCount: e.target.value
